  ...
  const performSearch = async (query) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/libros/?search=${encodeURIComponent(query)}`,
      );
  ...
  prestamoForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/prestamos/`, {
        ...
        body: JSON.stringify({
          libro: libroId,
          fecha_devolucion_esperada: calculateReturnDate(formData.get("dias")),
          observaciones: formData.get("notas"),
        }),
      });
      ...
      if (response.ok) {
        ...
        showNotification(
          `❌ Error: ${error.detail || "No se pudo crear el préstamo"}`,
          "error",
        );
      }
    } catch (error) {
...
const observeElements = () => {
  ...
  document
    .querySelectorAll(".stat-card, .feature-card, .libro-card")
    .forEach((el) => {
      observer.observe(el);
    });
};
